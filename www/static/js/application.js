...
-------------------------*/
			...
			$(document).on('portBox:open', function (e, box_id) {

				if (box_id !== portBox.attr("id")) {
					return false;
				}
...
-------------------------*/
			...
			$(document).on('portBox:close', function (e, box_id) {
...
--------------------------------*/
			...
			//Open
			$(document).trigger('portBox:open', [portBox.attr("id")])

			//Close
			//Close Button
			var closeButton = $('.close-portBox').one('click.portBoxEvent', function () {
				$(document).trigger('portBox:close', [portBox.attr("id")])
			});
			//Background Click
			if(options.bgclose) {
			...
			$('body').keyup(function(e) {
				if(e.which===27){ $(document).trigger('portBox:close', [portBox.attr("id")]); }
			});
...
Boolean.parse = function(obj, force_boolean) {
    force_boolean = force_boolean === undefined ? true : false;
    if (obj == null || obj == undefined || obj == [] || obj == {}) {
        return false;
    } else if (_.isArguments(obj)) {
        return obj.length > 0 ? true : false;
    }
    var str = obj.toString(),
        result_fn = force_boolean ? Boolean : _.identity;
    switch(str.toLowerCase()) {
        case "true": case "yes": return true;
        case "false": case "no": case null: return false;
        default: return result_fn(str);
    }
};
...
$.fn.extend({
    dattr: function(attr, val) {
        var my = $(this),
            attr = "data-" + attr;

        if (bool(val)) {
            return my.attr(attr, val);
        } else {
            return my.attr(attr);
        }
    },

    with_attr: function(attr, val) {
        if (bool(val)) {
            return $(this).filter("[" + attr + "='" + val + "']");
        } else {
            return $(this).filter("[" + attr + "]");
        }
    },

    with_dattr: function(attr, val) {
        return $(this).with_attr("data-" + attr, val);
    }
});
/*! jQuery UI - v1.10.3 - 2013-07-17
* http://jqueryui.com
...
(function() {
    ...
    var body = $("body"),
        ...
        body_freeze = function() {
            ...
            delta = body.innerWidth() - body_width;

            body.css('marginRight',
                (body.css('marginRight') ? '+=' : '') + delta);
        },
        body_unfreeze = function() {
            var body_width = body.innerWidth();
    ...
    var popups = $(".team.portBox").detach();

    $(".wrapper").append(popups);

    $(document).on("portBox:open", function(e) {
        body_freeze();
        $(document).on("mousewheel touchmove", freezer);
    }).on("portBox:close", function(e) {
        body_unfreeze();
        $(document).off("mousewheel touchmove", freezer);
    });

    window.alg_ns = {
        qs_to_obj: function(qs) {
            var decoded = decodeURI(qs),
                tokens = decoded.split("&"),
                result = {};

            _.each(tokens, function(v, i) {
                var pair = v.split("=");
        ...
        obj_to_qs: function(obj) {
            var result = [];

            _(obj).each(function(v, k, d) {
                result.push(encodeURIComponent(k) + "=" + encodeURIComponent(v));
            });

            return result.join("&");
        },
        manage_referer: function() {
            var tokens = window.location.href.split("?"),
            qs = tokens[1];

            var terms = alg_ns.qs_to_obj(qs);

            if (bool(terms.referer_url)) {
                $("#link-back").attr("href", terms.referer_url);
            }
        }
    };
